...
.factory('pushService',[ '$rootScope','$ionicPlatform','$cordovaPush','$http','pdaParams', 'cordovaReady','$ionicPopup','$cordovaMedia','clientConfig','Logger',
function ( $rootScope, $ionicPlatform, $cordovaPush, $http , pdaParams, cordovaReady, $ionicPopup, $cordovaMedia, clientConfig, Logger) {

	// "senderID" is "Project Number" from Google Developers Console (https://console.developers.google.com)
	// NOTE that tere is also a server component involved - see API key in /app/strongloop/OSC-API/server/gcm_config.js
  ...
  var androidConfig = {
	...
	var openPopup = true;
		...
		if (cordovaReady.isready && pdaParams.driverId > 0 && push_service.registered == false )
		{
			...
			log.debug('About to $cordovaPush.register: driver:' + push_service.mycurrent_drivernum);
			$cordovaPush.register(androidConfig).then(function(result) {
				...
				log.info('$cordovaPush.register for Driver ' + push_service.mycurrent_drivernum + ' : Success:', result);
